  ...
  function CrowdClient(settings) {
    ...
    this.search = {
      ...
      user: function user(restriction) {
        ...
        var startIndex = arguments.length <= 2 || arguments[2] === undefined ? 0 : arguments[2];
        var maxResults = arguments.length <= 3 || arguments[3] === undefined ? 1000 : arguments[3];
        var withAttributes = arguments.length <= 4 || arguments[4] === undefined ? false : arguments[4];

        restriction = encodeURIComponent(restriction);
        return _this.request('GET', '/search?entity-type=user&restriction=' + restriction + '&start-index=' + startIndex + '&max-results=' + maxResults + (expand ? '&expand=user' : '') + (withAttributes ? '&expand=attributes' : '')).then(function (res) {
